...
FROM
    (
        ...
        FROM
            eqtlinfo_prd.EQTL_MA.notas_servicos ns
            LEFT JOIN eqtlinfo_prd.EQTL_MA.visitas_notas vn
                ON vn.nota = ns.nota
            LEFT JOIN eqtlinfo_prd.EQTL_MA.info_gerais_notas ign
                ON ign.nota = ns.nota
            LEFT JOIN eqtlinfo_prd.EQTL_MA.tab_cadastro tc
                ON tc.instalacao = ns.instalacao
            LEFT JOIN eqtlinfo_prd.EQTL_MA.notas_fiscalizacao nf
                ON nf.nota = ns.nota
            LEFT JOIN eqtlinfo_prd.EQTL_MA.equipa_i_notas e
                ON ns.mandante = e.mandante
               AND ns.nota = e.nota
            LEFT JOIN SB_PERFORMANCE.EQTL_MA.tab_cod_irregularidade ir
                ON ir.irregularidade = nf.irregularidade
            LEFT JOIN eqtlinfo_prd.EQTL_MA.medidas_notas_servicos mns
                ON mns.nota = ns.nota
