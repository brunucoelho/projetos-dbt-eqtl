...
WITH ultima_atribuicao AS (
    ...
    FROM eqtlinfo_raw.oper_al.atribui_os_comercial
...
latitude_longitude AS (
    ...
    LEFT JOIN eqtlinfo_raw.oper_al.atribui_os_mensagem_retorno mr
...
FROM eqtlinfo_raw.oper_al.movto_os_comercial os
LEFT JOIN latitude_longitude ll ON ll.cd_movto_os_comercial = os.cd_movto_os_comercial
LEFT JOIN eqtlinfo_raw.oper_al.bairro br ON br.bairro_id = os.cd_bairro
LEFT JOIN eqtlinfo_raw.oper_al.localidade lo ON lo.lc_id = br.lc_id
LEFT JOIN eqtlinfo_raw.oper_al.municipio mp ON mp.mnc_id = lo.mnc_id
LEFT JOIN eqtlinfo_raw.oper_al.conclui_os_comercial cc ON cc.cd_movto_os_comercial = os.cd_movto_os_comercial
LEFT JOIN eqtlinfo_raw.oper_al.tipo_ocorrencia_corte oc ON oc.tpoco_corte_id = cc.cd_tp_ocorrencia_corte
LEFT JOIN eqtlinfo_raw.oper_al.atribui_os_comercial ac ON ac.cd_movto_os_comercial = os.cd_movto_os_comercial
LEFT JOIN eqtlinfo_raw.oper_al.prefixo_turma pt ON pt.prefixo_turma_id = ac.prefixo_turma_id
LEFT JOIN eqtlinfo_raw.oper_al.turma_os_comercial tc ON tc.atribuicao_os_id = ac.atribuicao_os_id
LEFT JOIN eqtlinfo_raw.oper_al.base bs ON bs.base_id = br.base_id
LEFT JOIN eqtlinfo_raw.oper_al.ponto_apoio pa ON pa.codigo_pa = bs.codigo_pa
LEFT JOIN eqtlinfo_raw.oper_al.regiao_eletrica rg ON rg.reg_eletrica_id = pa.reg_eletrica_id
LEFT JOIN eqtlinfo_prd.eqtL_al.tipo_os_local_religacao lr ON lr.tb_lcrelig_id = cc.cd_local_religacao
LEFT JOIN eqtlinfo_raw.oper_al.base bs3 ON bs3.base_id = mp.base_id
LEFT JOIN eqtlinfo_raw.oper_al.ponto_apoio pa3 ON pa3.codigo_pa = bs.codigo_pa
LEFT JOIN eqtlinfo_raw.oper_al.code_medida cm ON cm.ossubtipo_id = os.ossubtipo_id AND cc.cd_tp_conclusao_os = cm.code_medida_id
LEFT JOIN eqtlinfo_prd.eqtL_al.tipo_os_local_religacao lrel ON lrel.tb_lcrelig_id = cc.cd_local_religacao
