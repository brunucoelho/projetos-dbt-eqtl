...
with prazos as (
    select *
    from {{ ref("int_prazo_anexo_4__rs") }}
    {% if is_incremental() %}
...
visitas as (
    ...
    data_inicio_servico,
    data_final_servico,
    {{ normalize_prefix_model('equipe_execucao',"'RS'") }} equipe_execucao,
    matricula_executor,
    nome_executor
    from {{ ref("stg_eqtl_rs__visitas_notas") }}
)
...
where trunc(data_nota,'MM') >= '2026-04-01'