...
with mds_ns AS (
    ...
    FROM {{ source('eqtlinfo_prd_rs', 'medidas_notas_servicos')}} mns
), 
medidas_ns as (
    SELECT DISTINCT * 
    FROM mds_ns mdns
...
--OS.NOTA_VINCULADA
	FROM {{ source('eqtlinfo_prd_rs', 'notas_servicos')}} OS
    LEFT JOIN medidas_ns mns ON MNS.NOTA = OS.NOTA
    LEFT JOIN {{ source('eqtlinfo_prd_rs', 'tab_cadastro')}} c on os.instalacao = c.instalacao -- AND os.CONTA_CONTRATO = c.CONTA_CONTRATO 
    LEFT JOIN {{ source('eqtlinfo_prd_rs', 'tab_grpcodes_medidas')}} tgm ON MNS.GRUPO_MEDIDA = tgm.GRUPO_CODIGO
    LEFT JOIN {{ source('eqtlinfo_prd_rs', 'tab_status_notas')}} tsn ON OS.STATUS_CCS = TSN.STATUS_CCS AND OS.STATUSNTF2 = TSN.CODIGO_CCS
    LEFT JOIN {{ source('eqtlinfo_prd_rs', 'tab_codes_medidas')}} cm ON os.GRUPO_CODES = cm.GRUPO_CODIGO AND os.CODIFICACAO = cm.CODIGO AND os.TIPO_CATALOGO = cm.CATALOGO
    WHERE 21 = 21
