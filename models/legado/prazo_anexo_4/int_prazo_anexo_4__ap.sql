...
with mds_ns AS (
    SELECT *
    FROM {{ source('eqtlinfo_prd_ap', 'medidas_notas_servicos')}}
), 
medidas_ns as (
    SELECT DISTINCT * 
    FROM mds_ns mdns
    Where mdns.FIM_PROGRAMADO_MEDIDA BETWEEN TO_DATE('01/01/2024 00:00:00') AND DATEADD(MONTH,4,CURRENT_DATE())
),
source_data as (
SELECT DISTINCT
'EQTL_AP' AS EMPRESA,
...
--OS.NOTA_VINCULADA
	FROM {{ source('eqtlinfo_prd_ap', 'notas_servicos')}} OS
    LEFT JOIN medidas_ns mns ON MNS.NOTA = OS.NOTA
    LEFT JOIN {{ source('eqtlinfo_prd_ap', 'tab_cadastro')}} c on os.instalacao = c.instalacao -- AND os.CONTA_CONTRATO = c.CONTA_CONTRATO 
    LEFT JOIN {{ source('eqtlinfo_prd_ap', 'tab_grpcodes_medidas')}} tgm ON MNS.GRUPO_MEDIDA = tgm.GRUPO_CODIGO
    LEFT JOIN {{ source('eqtlinfo_prd_ap', 'tab_status_notas')}} tsn ON OS.STATUS_CCS = TSN.STATUS_CCS AND OS.STATUSNTF2 = TSN.CODIGO_CCS
    LEFT JOIN {{ source('eqtlinfo_prd_ap', 'tab_codes_medidas')}} cm ON os.GRUPO_CODES = cm.GRUPO_CODIGO AND os.CODIFICACAO = cm.CODIGO AND os.TIPO_CATALOGO = cm.CATALOGO
    WHERE 21 = 21
    -- b.data_final_servico BETWEEN to_date('05/01/2025 00:00:00', 'DD/MM/YYYY HH24:MI:SS')
	-- and to_date('20/04/2025 23:59:59', 'DD/MM/YYYY HH24:MI:SS')
