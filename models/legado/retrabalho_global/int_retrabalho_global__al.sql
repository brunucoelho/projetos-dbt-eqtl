...
WITH BASE_EMERGENCIAL AS (
    ...
    CASE 
    	WHEN rg.rel_descricao IN ('METROPOLITANA','NORTE') THEN 'LESTE'
    	WHEN rg.rel_descricao IN ('SERTAO','CENTRO') THEN 'OESTE'
    	ELSE ''
    END     AS regional,
    rg.rel_descricao AS SECCIONAL,
...
--AND pt.prx_descricao NOT ILIKE '%001%'
...
{{ normalize_prefix_model('prx.NAME',"'AL'") }}  AS prefixo,
...
ON (
    CASE 
        ...
        WHEN (E.DATA1 = '' AND E.DATA4 <> '') THEN 'AL.GA.' || E.DATA4
        ELSE E.DATA1
    END
) = SG.ID
...
WHERE PRM.Company = 'AL'
AND DATEADD(hour, -3, oii.OUTAGE_TIME) >= '2024-09-01'
...
TIPO_LOCALIZACAO_UC AS BAIRRO,
 ...
 B.BAS_NOME AS BASE,
  ACT.TX_EQ_INSTALACAO AS UC,
    {{ normalize_prefix_model('act.tx_resourceexternalid',"'AL'") }}  AS PREFIXO,
...
INNER JOIN eqtlinfo_raw.OPER_AL.municipio m ON m.mnc_id = act.nb_eq_codigomunicipio
INNER JOIN eqtlinfo_raw.OPER_AL.base b ON b.base_id = m.base_id
INNER JOIN eqtlinfo_raw.OPER_AL.ponto_apoio pa ON pa.codigo_pa = b.codigo_pa
INNER JOIN eqtlinfo_raw.OPER_AL.regiao_eletrica re ON re.reg_eletrica_id = pa.reg_eletrica_id
...
--INNER JOIN eqtlinfo_raw.oper_al.ponto_apoio pa ON pa.codigo_pa = b.codigo_pa
--INNER JOIN eqtlinfo_raw.oper_al.regiao_eletrica re ON re.reg_eletrica_id = pa.reg_eletrica_id
WHERE act.dt_activity >= '2024-09-01'
AND TX_STATE IN ('AL')
AND ACT.TX_EQ_INSTALACAO <> 0
    AND act.tx_status IN ('completed','complete')
   -- AND act.tx_resourceexternalid NOT IN ('PA_PA-BEL-S108M','PA_PA-BEL-S109M','PA_PA-BEL-S110M','PA_PA-BEL-S111M','PA_PA-BEL-S113M','PA_PA-BEL-S114M','PA_PA-BEL-S115M','PA_PA-BEL-S116M','PA_PA-BEL-S117M')
...
{{ normalize_prefix_model('NR_VIATURA',"'AL'") }}  AS PREFIXO,
NULL AS REGISTRO_EXEC,
