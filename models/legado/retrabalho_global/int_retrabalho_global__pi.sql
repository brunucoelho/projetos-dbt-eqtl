...
rg.rel_descricao AS regional,
    ...
    {{ normalize_prefix_model('pt.prx_descricao',"'AL'") }} AS prefixo,
...
CASE
 ...
 FROM
    EQTLINFO_RAW.OPER_PI.ocorrencia oc
left JOIN  EQTLINFO_RAW.OPER_PI.tipo_de_subcausa sub on sub.tb_subcausa_id = oc.tb_subcausa_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.tipo_de_causa tc ON tc.tb_causa_id = oc.tb_causa_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.historico_ocorrencia_turma ot ON ot.ocorrencia_id = oc.ocorrencia_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.historico_turma_plantao tp ON tp.hist_turma_plantao_id = ot.hist_turma_plantao_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.prefixo_turma pt ON pt.prefixo_turma_id = tp.prefixo_turma_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.base bs ON bs.base_id = oc.base_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.ponto_apoio pa ON pa.codigo_pa = bs.codigo_pa
    --INNER JOIN EQTLINFO_RAW.OPER_PI.unidade_territorial ut ON ut.reg_eletrica_id = pa.reg_eletrica_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.regiao_eletrica rg ON rg.reg_eletrica_id = pa.reg_eletrica_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.bairro br ON br.bairro_id = oc.bairro_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.localidade lo ON lo.lc_id = br.lc_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.municipio mp ON mp.mnc_id = lo.mnc_id
WHERE
    oc.oco_status = 'F'
    ...
    AND oc.oco_data_conclusao >= TO_DATE('01/10/2023', 'DD/MM/YYYY')
       ...
       AND  pa.pap_nome    NOT ILIKE ('%DESPACHO%')      
       AND OC.TB_NATUREZA_ID = 5
AND  pa.codigo_pa < 100
AND oc.tb_tp_abrangencia = 'CR'
AND TIPO = 'P'
AND PREFIXO NOT ILIKE '%-H0%'
AND PREFIXO NOT IN ('THEC0','SOL001','SUM01','NORM05','NORM03','SUM30','NORM1','METM78','SUDM0','SUM7','METM0','SUM29','NORM02','THEM14','SUM0','SUM02','THEM11','THEM20','SUM04','THEM0','THEM34','NORM0','NORM04','NORM01')
...
BASE_COMERCIAL AS (
    ...
    FROM
        EQTLINFO_RAW.OPER_PI.atribui_os_comercial ac2
    GROUP BY
        ac2.cd_movto_os_comercial
)
SELECT
    ...
    {{ normalize_prefix_model('PT.PRX_DESCRICAO',"'PI'") }} AS prefixo,     
...
FROM
    EQTLINFO_RAW.OPER_PI.movto_os_comercial os
    INNER JOIN EQTLINFO_RAW.OPER_PI.bairro br ON br.bairro_id = os.cd_bairro
    INNER JOIN EQTLINFO_RAW.OPER_PI.localidade lo ON lo.lc_id = br.lc_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.municipio mp ON mp.mnc_id = lo.mnc_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.conclui_os_comercial cc ON cc.cd_movto_os_comercial = os.cd_movto_os_comercial
    LEFT JOIN EQTLINFO_RAW.OPER_PI.tipo_ocorrencia_corte oc ON oc.tpoco_corte_id = cc.cd_tp_ocorrencia_corte
    INNER JOIN EQTLINFO_RAW.OPER_PI.atribui_os_comercial ac ON ac.cd_movto_os_comercial = os.cd_movto_os_comercial
    INNER JOIN EQTLINFO_RAW.OPER_PI.prefixo_turma pt ON pt.prefixo_turma_id = ac.prefixo_turma_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.turma_os_comercial tc ON tc.atribuicao_os_id = ac.atribuicao_os_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.base bs ON bs.base_id = br.base_id
    INNER JOIN EQTLINFO_RAW.OPER_PI.ponto_apoio pa ON pa.codigo_pa = bs.codigo_pa
    INNER JOIN EQTLINFO_RAW.OPER_PI.regiao_eletrica rg ON rg.reg_eletrica_id = pa.reg_eletrica_id
    --LEFT JOIN EQTLINFO_PRD.EQTL_GO.tipo_os_local_religacao lr ON lr.tb_lcrelig_id = cc.cd_local_religacao
    INNER JOIN EQTLINFO_RAW.OPER_PI.code_medida cm ON cm.ossubtipo_id = os.ossubtipo_id AND cc.cd_tp_conclusao_os = cm.code_medida_id
    --LEFT JOIN EQTLINFO_PRD.EQTL_GO.tipo_os_local_religacao lrel ON lrel.tb_lcrelig_id = cc.cd_local_religacao
...
QUALIFY
   ...
   -- AND tipo = 'P'
    ...
    AND os.ostipo_id NOT IN ('CT','DS','NT')  
    AND pt.prx_descricao NOT IN ('SUM01','NORM05','NORM03','SUM30','NORM1','METM78','SUDM0','SUM7','METM0','SUM29','NORM02','THEM14','SUM0','SUM02','THEM11','THEM20','SUM04','THEM0','THEM34','NORM0','NORM04','NORM01')
   AND TIPO = 'P'
),
BASE_PERDAS AS(
SELECT 
...
{{ normalize_prefix_model('NR_VIATURA',"'PI'") }} AS PREFIXO,
