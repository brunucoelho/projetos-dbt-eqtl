...
WITH BASE_EMERGENCIAL AS (
    ...
    {{ normalize_prefix_model('pt.prx_descricao',"'AP'") }} AS prefixo,
...
FROM EQTLINFO_RAW.OPER_AP.ocorrencia oc

LEFT JOIN EQTLINFO_RAW.OPER_AP.tipo_de_subcausa sub
       ON sub.tb_subcausa_id = oc.tb_subcausa_id

INNER JOIN EQTLINFO_RAW.OPER_AP.tipo_de_causa tc
        ON tc.tb_causa_id = oc.tb_causa_id

INNER JOIN EQTLINFO_RAW.OPER_AP.historico_ocorrencia_turma ot
        ON ot.ocorrencia_id = oc.ocorrencia_id

INNER JOIN EQTLINFO_RAW.OPER_AP.historico_turma_plantao tp
        ON tp.hist_turma_plantao_id = ot.hist_turma_plantao_id

INNER JOIN EQTLINFO_RAW.OPER_AP.prefixo_turma pt
        ON pt.prefixo_turma_id = tp.prefixo_turma_id

INNER JOIN EQTLINFO_RAW.OPER_AP.base bs
        ON bs.base_id = oc.base_id

INNER JOIN EQTLINFO_RAW.OPER_AP.ponto_apoio pa
        ON pa.codigo_pa = bs.codigo_pa

INNER JOIN EQTLINFO_RAW.OPER_AP.regiao_eletrica rg
        ON rg.reg_eletrica_id = pa.reg_eletrica_id

INNER JOIN EQTLINFO_RAW.OPER_AP.bairro br
        ON br.bairro_id = oc.bairro_id

INNER JOIN EQTLINFO_RAW.OPER_AP.localidade lo
        ON lo.lc_id = br.lc_id

INNER JOIN EQTLINFO_RAW.OPER_AP.municipio mp
...
{{ normalize_prefix_model('prx.NAME',"'AP'") }} AS prefixo,
...
WHERE PRM.Company = 'AP'
AND DATEADD(hour, -3, oii.OUTAGE_TIME) >= '2025-06-01'
...
TIPO_LOCALIZACAO_UC AS BAIRRO,
 ...
 B.BAS_NOME AS BASE,
  ACT.TX_EQ_INSTALACAO AS UC,
    {{ normalize_prefix_model('act.tx_resourceexternalid',"'AP'") }} AS PREFIXO,
...
LEFT JOIN eqtlinfo_raw.OPER_AP.code_medida cm ON
...
INNER JOIN eqtlinfo_raw.OPER_AP.municipio m ON m.mnc_id = act.nb_eq_codigomunicipio
INNER JOIN eqtlinfo_raw.OPER_AP.base b ON b.base_id = m.base_id
INNER JOIN eqtlinfo_raw.OPER_AP.ponto_apoio pa ON pa.codigo_pa = b.codigo_pa
INNER JOIN eqtlinfo_raw.OPER_AP.regiao_eletrica re ON re.reg_eletrica_id = pa.reg_eletrica_id
...
--INNER JOIN eqtlinfo_raw.oper_ap.ponto_apoio pa ON pa.codigo_pa = b.codigo_pa
--INNER JOIN eqtlinfo_raw.oper_ap.regiao_eletrica re ON re.reg_eletrica_id = pa.reg_eletrica_id
WHERE act.dt_activity >= '2025-06-01'
AND TX_STATE IN ('AP')
AND ACT.TX_EQ_INSTALACAO <> 0
    AND act.tx_status IN ('completed','complete')
   -- AND act.tx_resourceexternalid NOT IN ('PA_PA-BEL-S108M','PA_PA-BEL-S109M','PA_PA-BEL-S110M','PA_PA-BEL-S111M','PA_PA-BEL-S113M','PA_PA-BEL-S114M','PA_PA-BEL-S115M','PA_PA-BEL-S116M','PA_PA-BEL-S117M')
...
{{ normalize_prefix_model('NR_VIATURA',"'AP'") }} AS PREFIXO,
NULL AS REGISTRO_EXEC,
